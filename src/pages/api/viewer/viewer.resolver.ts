...
import {
  ...
  ConnectPayoutInput, GetDiscountInput,
  DiscountProductType, RemoveEnrollmentInput, RegisterEventInput, ConnexionProfileType,
  PROFILE_STATUS, PROFILE_STATUS_ENUM,
  UpdateViewerStatusInput, UpdateViewerStatusOutput, AccountStatusInput, EnrollmentType, EnrollmentInput, EventTypeData, UpdateEventInput, RemoveEventInput,
} from './viewer.types';
    ...
    if (id) {
      ...
      const profileSnapshot= await profileRef.get()
       ...
       typeof profileSnapshot?.data()?.events !== 'undefined') {
            ...
            return { success: false,   events:[{id: '',
...
const removeViewer = async (
  _: undefined,
  { email }: { email: string },
  { ViewerModel, res, oneWeek }: { oneWeek: unknown,
...
const payEnrollment = async (
  ...
  try {
     const doc = await ViewerModel.findOne({ email: collaboratorEmail }).exec() 
      ...
      return {
            success: false, message: JSON.stringify({title,  price, status, startDate: collaboratorExists.startDate ?
          ...
          await doc?.save();
          return {
            success: true,message: JSON.stringify({ title, profileEmail,  price, startDate, status, profileId,
        ...
        if (organisatorExists && organisatorExists.length === 1 && organisatorExists[0].token !== '') {
          ...
          return {
            success: false, message: JSON.stringify({title,  price, status, startDate: organisatorExists.startDate ?
        ...
        if (discountExists && discountExists.length === 1 && discountExists[0].token !== '') {
          ...
          return {
            success: false,message: JSON.stringify({ title,  price, status, startDate: discountExists.startDate ?
      ...
      if (libraryExists && libraryExists.length === 1 && libraryExists[0].token !== '') {
          const startD = new Date().toISOString()
        return {
            success: false,message: JSON.stringify({ title,  price, status, startDate: libraryExists.startDate ?
    ...
    if (liisExists && liisExists.length === 1 && liisExists[0].token !== '') {
          const startD = new Date().toISOString()
      return {
            success: false, message: JSON.stringify({title,  price, status, startDate: liisExists.startDate ?
        ...
        if (liismaiilExists && liismaiilExists.length === 1 && liismaiilExists[0].token !== '') {
          const startD = new Date().toISOString()
        return {
            success: false,  message: JSON.stringify({title,  price, status, startDate: liismaiilExists.startDate ?
      ...
      if (guestExists && guestExists.length === 1 && guestExists[0].token !== '') {
          const startD = new Date().toISOString()
          return {
            success: false, message: JSON.stringify({title,  price, status, startDate: guestExists.startDate ?
...
module.exports = {
  ...
  Query: {
    ...
    getEvents,
    getDiscountProfiles,
    getLiismaiilProfiles,
    getLibraryProfiles,
    getLiismanagerProfiles,
    getCollaboratorProfiles,
    getOrganisatorProfiles,
    getDeliverProfiles,
    getQrCode,
    getDiscountProducts,
