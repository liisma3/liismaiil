import { ProductInput, ProductTypeData, UpdateProductInput, RemoveProductsInput, PromoteProductInput } from './product.types';
...
const discounts = async (
  ...
  try {
    const products: Array<ProductTypeData> = [];
    return dbFirestore.collection('products').where('discount', '==', true).get().then((querySnapshot: unknown) => {
      querySnapshot.forEach((doc: unknown) => {
        products.push({ id: doc.id, ...doc.data() });
        /*   console.log({ productDoc: doc.data() }) */
      });
...
const vracs = async (
  ...
  try {
    const products: Array<ProductTypeData> = [];
    return dbFirestore.collection('products').where('productStatus', 'array-contains', 'VRAC').get()
...
const promoteProduct = async (
  ...
  try {
    await dbFirestore.collection('products').doc(titleSlug).update({
      poromotedBy: FieldValue.arrayUnion({ profile, rate })
    });
    try {
      await dbFirestore.collection('profiles').doc(profile).update({
        productsPromoted: FieldValue.arrayUnion({
          selectionSlug, titleSlug, rate
        })
      });
    } catch (error) {
