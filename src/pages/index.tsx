...
export default function Home({ organisations, products }: { organisations: ViewerTypeData[], products: ProductTypeData[] }) {
  useEffect(() => {
    window.scrollTo(0, 0)
  }, [])
  const countries = organisations.map((org) => {
    return org.country
  })
  ...
  return (
    ...
    <main className="bg-gray-200 mt-20  max-w-screen-2xl justify-center items-center" >
      <header className='h-full   bg-gradient-to-r from-[#0f1f47] to-[#5f6984]'>
        ...
        <Organisations organisations={organisations} countries={countries} cities={cities} />
      </header>
...
export async function getStaticProps() {
  ...
  productsArray.forEach((doc: ProductTypeData) => {
    ...
    products.push({
      title, description, price, productStatus,
      promo, promotedBy, stock, image, author
    });
  });
